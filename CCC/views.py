...
def checkout(request):
    ...
    if request.method == 'POST':
        if 'user' in request.session:
            ...
            order = customer_order(fname=fname,lname=lname,address=address,oaddress=oaddress,city=city,state=state,zipcode=zipcode,email=email,mobile=mobile,ordernote=ordernote,product=data,Customer_id=cust.id)
            order.save()
